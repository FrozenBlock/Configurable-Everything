...
internal sealed class ScriptType(val clientOnly: Boolean) {
    data object CLIENT : ScriptType(true)
    data object COMMON : ScriptType(false)
}
...
object ScriptingUtil {
    ...
    private suspend fun compileScript(script: File, type: ScriptType, addToBuffer: Boolean = true): Pair<CompiledScript, File>? {
        ...
        if (type.clientOnly && !ModLoader.isClient()) return null
